...
def process_card(ccx):
    ...
    try:
        ...
        if 'id' not in pm_data:
            ...
            return {"cc": ccx, "response": error_msg, "status": "Declined"}

        payment_method_id = pm_data['id']
    except Exception as e:
        return {"cc": ccx, "response": f"Payment Method Creation Failed: {str(e)}", "status": "Declined"}
    ...
    try:
        ...
        if 'createAndConfirmSetupIntentNonce' in nonce_response.text:
            ...
            return {"cc": ccx, "response": "Failed to extract nonce", "status": "Declined"}
    except Exception as e:
        return {"cc": ccx, "response": f"Nonce Retrieval Failed: {str(e)}", "status": "Declined"}
    ...
    try:
        ...
        if setup_data.get('success', False):
            ...
            if data_status == 'requires_action':
                return {"cc": ccx, "response": "Action Required", "status": "Approved"}
            elif data_status == 'succeeded':
                return {"cc": ccx, "response": "Succeeded", "status": "Approved"}
        ...
        return {"cc": ccx, "response": str(setup_data), "status": "Declined"}

    except Exception as e:
        return {"cc": ccx, "response": f"Setup Intent Failed: {str(e)}", "status": "Declined"}


@app.route('/gate=txtchk/key=wasdarkboy/cc=<path:cc>')
def check_card(cc):
    if not re.match(r'^\d{13,19}\|\d{1,2}\|\d{2,4}\|\d{3,4}$', cc):
        return jsonify({"cc": cc, "response": "Invalid card format", "status": "Error"}), 400
